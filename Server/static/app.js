...
const personalityQuizDrawerOpenEl = document.getElementById("personality-quiz-drawer-open");
...
const personalityQuizModalEl = document.getElementById("personality-quiz-modal");
const personalityQuizCloseEl = document.getElementById("personality-quiz-close");
const personalityQuizFormEl = document.getElementById("personality-quiz-form");
const personalityQuizStatusEl = document.getElementById("personality-quiz-status");
...
const MOBILE_STICKY_NOTE_ENTRY_DISTANCE = 169;
...
function ensureStickyNoteDragHost() {
  if (stickyNoteDragHostEl instanceof HTMLElement) return stickyNoteDragHostEl;
  stickyNoteDragHostEl = document.createElement("div");
  stickyNoteDragHostEl.className = "sticky-note-drag-host";
  document.body.appendChild(stickyNoteDragHostEl);
  return stickyNoteDragHostEl;
}

function moveStickyNoteToDragHost(noteEl) {
  const dragHostEl = ensureStickyNoteDragHost();
  if (!(dragHostEl instanceof HTMLElement)) return;
  dragHostEl.appendChild(noteEl);
}

function restoreStickyNoteFromDragHost(noteEl) {
  if (!stickyNoteLayerEl || noteEl.parentElement !== stickyNoteDragHostEl) return;
  if (noteEl.classList.contains("is-stowed") && isTouchDevice && stickyNoteMobileContentEl instanceof HTMLElement) {
    stickyNoteMobileContentEl.appendChild(noteEl);
    return;
  }
...
function positionStickyNoteDockSlot(index, draggingNoteEl = null) {
  ...
  let slotWidth = isExpandedStowArea ? STICKY_NOTE_HOVER_REVEAL_WIDTH : slotSize.width;
  let stowedLeft = Math.max(0, window.innerWidth - slotWidth);
  if (isTouchDevice) {
    const sampleHeight = Math.max(STICKY_NOTE_STOWED_HEIGHT, Math.round(slotSize.height));
    const visibleStackHeight = Math.max(
      sampleHeight,
      (Math.max(1, MOBILE_STICKY_NOTE_VISIBLE_COUNT) - 1) * stackGap + sampleHeight
    );
    const viewportHeight = Math.min(
      window.innerHeight,
      Math.round(stackTopStart + visibleStackHeight + sampleHeight + MOBILE_STICKY_NOTE_ENTRY_DISTANCE)
    );
    const previewBaseTop = stackTopStart + index * stackGap;
    const previewViewportY = previewBaseTop - stickyNoteMobileScrollTop;
    const revealProgress = clamp((viewportHeight - previewViewportY) / MOBILE_STICKY_NOTE_ENTRY_DISTANCE, 0, 1);
    ...
    const baseStowedLeft = STICKY_NOTE_DEFAULT_WIDTH - getMobilePeekWidth();
    const noteLocalLeft = Math.round(baseStowedLeft + revealOffset);
    const visibleWidth = clamp(STICKY_NOTE_DEFAULT_WIDTH - noteLocalLeft, 0, STICKY_NOTE_DEFAULT_WIDTH);
    slotWidth = Math.max(0, Math.round(visibleWidth));
    stowedLeft = Math.round(window.innerWidth - STICKY_NOTE_DEFAULT_WIDTH + noteLocalLeft);
  }
...
function createStickyNote(listEntry, colorClassName) {
  ...
  function beginStickyNoteDrag(pointerState) {
    const rect = noteEl.getBoundingClientRect();
    moveStickyNoteToDragHost(noteEl);
    noteEl.style.transition = "";
    ...
    if (isTouchDevice) {
      setMobilePeekExpanded(startsNearDock);
    }
  ...
  noteEl.addEventListener("pointerdown", (event) => {
    ...
    if (isTouchDevice && isStowedAtPointerDown) {
      clearTouchIntent();
      touchIntentState = {
        pointerId: event.pointerId,
  ...
  noteEl.addEventListener("pointermove", (event) => {
    ...
    if (isTouchDevice) {
      setMobilePeekExpanded(effectiveNearDock);
    }
    if (effectiveNearDock) {
  ...
  function releaseStickyNote(event) {
    ...
    if (shouldStow) {
      ...
      restoreExpandedSizeFromPreference();
      restoreStickyNoteFromDragHost(noteEl);
      layoutStowedStickyNotes();
...
function setPersonalityQuizStatus(text, isError = false) {
  if (!personalityQuizStatusEl) return;
  personalityQuizStatusEl.textContent = text || "";
  personalityQuizStatusEl.classList.toggle("is-error", Boolean(isError && text));
}
...
function openPersonalityQuiz() {
  ...
  if (!personalityQuizModalEl) return;
  closeNavDrawer();
  personalityQuizModalEl.classList.remove("hidden");
  setPersonalityQuizStatus("");
}

function closePersonalityQuiz() {
  if (!personalityQuizModalEl) return;
  personalityQuizModalEl.classList.add("hidden");
}

function buildCommunicationProfileFromQuiz() {
  if (!personalityQuizFormEl) return null;
  const data = new FormData(personalityQuizFormEl);
  const teachingStyle = String(data.get("teaching_style") || "");
  const format = String(data.get("format") || "");
  return {
    type: "communication_profile",
    verbosity: String(data.get("verbosity") || ""),
    tone: String(data.get("tone") || ""),
    format,
    teaching_style: teachingStyle === "analogies" && format === "examples"
      ? "analogies_then_examples"
      : teachingStyle,
    challenge_level: String(data.get("challenge_level") || ""),
    correction_style: String(data.get("correction_style") || ""),
    emotional_support: String(data.get("emotional_support") || ""),
    decision_support: String(data.get("decision_support") || "")
  };
}

async function saveCommunicationProfile(event) {
  event.preventDefault();
  if (!isAuthenticated) {
    setPersonalityQuizStatus("Sign in first.", true);
    return;
  }
  const profile = buildCommunicationProfileFromQuiz();
  if (!profile) return;
  setPersonalityQuizStatus("Saving...");
  try {
    const response = await fetch("/api/communication-profile", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    setPersonalityQuizStatus(error.message || "Unable to save profile.", true);
  }
}
...
function updateAuthUi() {
  ...
  if (personalityQuizDrawerOpenEl) {
    personalityQuizDrawerOpenEl.disabled = !isAuthenticated;
  }
}
...
if (personalityQuizDrawerOpenEl) {
  personalityQuizDrawerOpenEl.addEventListener("click", openPersonalityQuiz);
}

if (personalityQuizCloseEl) {
  personalityQuizCloseEl.addEventListener("click", closePersonalityQuiz);
}

if (personalityQuizModalEl) {
  personalityQuizModalEl.addEventListener("click", (event) => {
    if (event.target === personalityQuizModalEl) {
      closePersonalityQuiz();
    }
  });
}

if (personalityQuizFormEl) {
  personalityQuizFormEl.addEventListener("submit", saveCommunicationProfile);
}
